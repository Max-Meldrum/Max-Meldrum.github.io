...
Arcon has until recently been a pure row-based (Protobuf) system. While this data arrangement works for a range of streaming operations, it is not a suitable for ad-hoc OLAP queries or larger bulk aggregations on streaming windows.
...
It therefore makes sense for Arcon join the Arrow ecosystem so that it can benefit both from Columnar processing and the 
