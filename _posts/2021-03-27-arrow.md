...
Converting the ``Event`` struct to Arrow data is not straightfoward.
The Arrow [crate](https://github.com/apache/arrow/tree/master/rust/arrow) provides a bunch of low-level builder types that one can use to append values to. Let's first go through an example where we build everything manually.
...
It is in the ``table`` method that we construct the StructBuilder. As long as we provide the StructBuilder with all the correct Arrow Fields, it will then take care of initialising every child field builder.
...
A ``MutableTable`` then uses the append method of ``ToArrow`` each time data is added to the table. The derive macro will then expand the append method of the ``Event`` struct into the following code:
...
So rather working with low-level builders, we work with tables (``MutableTable``, ``ImmutableTable``, and ``RawTable``) that hides the complexity of creating Arrow data.

``` rust
let event = Event { f1: 1, f2: String::from("text"), f3: 1.0 };
let mutable_table = Event::table();
mutable_table.append(event);
```

## Fixed Limitations

At first we were not able to support ``String`` and ``Vec<u8>`` as their ArrayBuilder's were constrained to accept values as references while our macro passes things by value. This issue is now fixed as a [patch](https://github.com/apache/arrow/pull/9570) was contributed upstream to Arrow.
